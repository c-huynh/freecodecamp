...
suite('UnitTests', () => {
    suiteSetup(() => {
        ...
        return JSDOM.fromFile('./views/index.html', options).then((dom) => {
    ...
    suite('Function isValidInput()', () => {
        test('Valid "1-9" characters', (done) => {
            ...
            input.forEach(i => {
                assert.equal(Solver.isValidInput(i), true);
            })
            done();
        });
        ...
        test('Invalid characters (anything other than "1-9") are not accepted', (done) => {
            ...
            input.forEach(i => {
                assert.equal(Solver.isValidInput(i), false);
            })
            done();
        });
    });

    suite('Function parseSudokuStr()', () => {
        test('Parses a valid puzzle string into an object', done => {
            ...
            assert.deepStrictEqual(Solver.parseSudokuStr(input).value, expected.value);
            done();
        });
        ...
        test('Shows an error for puzzles that are not 81 numbers long', done => {
            ...
            Solver.parseSudokuStr(shortStr);
            assert.equal(errorDiv.innerHTML, errorMsg);
            
            Solver.parseSudokuStr(longStr);
            assert.equal(errorDiv.innerHTML, errorMsg);
    ...
    suite('Function isValidPuzzle()', () => {
        // Valid complete puzzles pass
        test('Valid puzzles pass', done => {
            const input = '769235418851496372432178956174569283395842761628713549283657194516924837947381625';
            const puzzle = Solver.parseSudokuStr(input);
            assert.equal(Solver.isValidPuzzle(puzzle), true);
            done();
        });
        ...
        test('Invalid puzzles fail', done => {
            ...
            const puzzle = Solver.parseSudokuStr(input);
            assert.equal(Solver.isValidPuzzle(puzzle), false);
            done();
        });
    });
    ...
    suite('Function solve()', () => {
        ...
        test('Returns the expected solution for an incomplete puzzle', done => {
            ...
            const grid = Solver.parseSudokuStr(input).value;
            assert.deepStrictEqual(Solver.solve(grid), Solver.parseSudokuStr(output).value);
            done();
        });
    });
});
